...
def can_leave_room(puzzle, room_pos, room):
  if all(puzzle[pos] == room for pos in room_pos if puzzle[pos] != '.'): return False
  for a in room_pos:
...
def possible_moves(puzzle, parc, stepout, target):
  for a in [pos for pos in parc if puzzle[pos] != '.']:
    ...
    if (b := can_enter_room(a, stepout[amphi], amphi, puzzle, target[amphi])):
      yield a, b
  for room in 'ABCD':
    if not (a := can_leave_room(puzzle, target[room], room)): continue
...
def solve(puzzle):
  ...
  while heap:
    ...
    for a, b in possible_moves(state, parc, stepout, target):
      ...
      if new_cost >= seen.get(new_state, 999999): continue
      seen[new_state] = new_cost
      heappush(heap, (new_cost, new_state))


print(solve(read_puzzle("C:/github/AdventOfCode/2021/Day23/input.txt")))
print(solve(read_puzzle("C:/github/AdventOfCode/2021/Day23/input2.txt")))